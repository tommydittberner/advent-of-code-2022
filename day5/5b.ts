...
function moveCrane(stacks: string[][], orders: number[]): string[][] {
    for (let i = 0; i < orders.length; i += 3) {
        ...
        let fromStack = stacks[orders[i + 1] - 1];
        let toStack = stacks[orders[i + 2] - 1];
        
        let toMove = fromStack.splice(fromStack.length - numToMove, fromStack.length);
        toStack = [...toStack, ...toMove];
